...
def add_info(conn,a,b,c):

    cur = conn.cursor()
    cur.execute("UPDATE traineeRecord SET Finished_Task = ?, Self_Rating = ? WHERE Trainee_ID = ?",(a,b,c,))
    return 1
...
class User(Resource):
    ...
    def put(selfself,name):
        ...
        with conn:
            ##
            result = add_info(conn, args["Finished_Task"], args["Self_Rating"], name)
            if result == 1:
                return 200
