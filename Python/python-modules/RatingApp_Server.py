        ...
        while True:
            ...
            with socketConnection:
                ...
                if choice == "1":
                    ...
                    received_record = requests.get("http://127.0.0.1:5000/user/" + str(received_qr_code) + "?Date="+received_date)
                    ...
                    received_date = socketConnection.recv(1024).decode('utf-8')
                    print("date = ", received_date)                                     # receive date
                    # those were unencrypted
                    #### "&Date="+str(received_date)+
                    statusSigning = "unsigned"
                    execute_update_task = requests.put("http://127.0.0.1:5000/user/"+str(ID_Returned)+"?Finished_Task="+received_finishedTask+"&Self_Rating="+received_selfRating+"&ID_Supervisor="+str(received_supervisorID)+"&Date="+str(received_date)+"&QR_Code="+str(QR_Code)+"&Signing_Status="+str(statusSigning))
                    print("//check/")
                    ...
                    received_date = socketConnection.recv(1024).decode('utf-8') # receive

                    received_qr_code = int(socketConnection.recv(1024).decode('utf-8')) # receive

                    record_shared = see_record(dbConnection,received_qr_code,received_date)

                    record_shared_dump = json.dumps(record_shared)
                    port = 587  # For starttls
                    smtp_server = "smtp.gmail.com"
                    sender_email = "frankwoods1243@gmail.com"
                    receiver_email = "810.hoangchuong@gmail.com"
                    check_ = "check"
                    ...
                    assessment_update = requests.put("http://127.0.0.1:5000/user/"+str(received_assessQR)+"?Finished_Task="+str(e1)+"&Self_Rating="+str(e2)+"&Supervisor_Rating="+str(e3)+"&Supervisor_FeedBack="+str(e4)+"&Signing_Status="+str(signStatusUpdated)+"&Record_Hashing="+hashedRecord)
                    ...
                    if check_condition == "employee":
                        response = requests.get('http://127.0.0.1:5000/user/'+check_condition+'?ID_Employee='+str(data1)+'&Employee_Name='+data2)  # check employee
                        a = response.content
                        b = a.decode('utf-8')
                        b = b.replace('"', '')
                        socketConnection.send(b.encode())
                    elif check_condition == "hash":
                        response = requests.get('http://127.0.0.1:5000/user/'+check_condition+'?QR_Code='+str(data1)+'&Trainee_ID='+str(data2))  # check hash
                        a = response.content
                        b = a.decode('utf-8')
                        b = b.replace('"', '')
                        socketConnection.send(b.encode())
