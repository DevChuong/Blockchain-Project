...
def add_info(conn,a,b,c,d,e,f):
    ...
    cur = conn.cursor()
    cur.execute("update traineeRecord set Finished_Task = ? , Self_Rating = ? , Supervisor_Rating = ? , Supervisor_FeedBack = ? , Signing_Status = ? where QR_Code = ?" ,(a,b,c,d,e,f,))
    return 1
...
class User(Resource):
    ...
    def put(selfself,name):
        ...
        with conn:
            ##
            result = add_info(conn, args["Finished_Task"], args["Self_Rating"], args["Supervisor_Rating"], args["Supervisor_FeedBack"],args["Signing_Status"], name)
            if result == 1:
                return 200
